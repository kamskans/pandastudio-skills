...
- **Entrances must be MASKED or CONTINUOUS (engine 0.7.17).** The engine shows an element at its CSS value except *during* a tween's active window and reverts to CSS after — so a bare `from`/`fromTo` entrance on un-masked content FLICKERS (visible → snap to `from` → re-enter), and CSS-hidden + `.to` reverts to hidden. Either play entrances under a covering wipe (content CSS-visible), or skip staged reveals and animate continuously over the full `data-duration`. See motion-philosophy.md Rule 12 — this is the #1 "static/flicker" bug.
