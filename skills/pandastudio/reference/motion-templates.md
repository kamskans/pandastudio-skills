...
- `sh-serif-quote` `O` (6s, Shorts; for 16:9 long-form use `serif-statement`) — the one sentence that carries the idea, warm serif, words revealing in speaking rhythm; phrases wrapped in `*asterisks*` turn gold italic. Place on the moment the speaker says it. Slots: **quote**, inkColor, accentColor.
...
- `pill-list` (10s) — a short list of THINGS (tools, gear, traits): 2 to 6 rounded off-white pills (Inter ~60px, a blue/violet/coral dot each) with an optional title/label, each landing on its word via `at`, plus an optional illustration beside them (`image`, fades in at `imageAt`). For ordered steps use `numbered-slide`. Illustration: `media.generate-image` a simple flat illustration on an off-white ground with no text, cut it out to a transparent PNG if you can (an uncut one still shows as a rounded card). Long pills wrap to two lines. Slots: items[{text, at}], image, imageAt, title, eyebrow, cameraSide, pillColor, color1-3, eyebrowColor, bgColor, inkColor. Placed like the other camera-card slides.
