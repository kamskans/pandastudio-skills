...
| `project.duplicate-region` | `regionType` (zoom/speed/annotation/fx/overlay/clip-transform/background-effect/spotlight/motion/adjustment/motion-element/audio-overlay), `regionId`, `atMs?` | Copy a region with all its settings and a new id, right after the original or at `atMs` (EDITED ms; SOURCE ms for speed). Link groups copy as a new group. Zooms/speed shift to the next free gap (`shiftedMs`) or fail when none fits. |
...
| `project.add-transition` | `transitionId` (bundled) OR `file`, `atMs` (req, the cut time), `durationMs` (default: catalog; native 120-2000), `snapMs` (default 500), `sound` (default the catalog sound, `none` = silent), `soundVolume` | Place a transition centered on a cut (clip boundary or jump cut). Overlay kind draws a WebM over the cut; native kind (zoom-blur, whip-left/right/up, spin) moves the footage itself, captions and graphics stay put. Returns `regionId`, `kind`, `effect`, `centerMs`, `snappedToCut`. See fx-transitions.md. |
...
| Command | Args | Purpose |
|---|---|---|
| `media.generate-image` | `prompt` (required), `aspectRatio` (`1:1` \| `3:2` (default) \| `2:3` \| `4:3` \| `3:4` \| `16:9` \| `9:16`), `quality` (`low` \| `medium` (default) \| `high`), `referenceImagePath` (path/URL, optional), `transparent` (bool: trimmed PNG sticker with the background removed), `provider` (`auto` (default) \| `replicate` \| `higgsfield`), `outputName` (slug, optional) | Generate one image on the user's OWN image connector: `auto` = Replicate (openai/gpt-image-2) when connected, else Higgsfield (gpt_image_2_5); billed to their credits there, and each call is a paid generation (never repeat one). Returns `{ imagePath, prompt, aspectRatio, provider, model, generationId, predictionId, transparent, transparency?, warnings? }` (`transparency`: `native` \| `keyed` \| `none`). No image connector → fails with `details.code: "NO_IMAGE_CONNECTOR"`: ask the user for their own images (or use images in the project folder) instead. A named provider that isn't connected → `PROVIDER_NOT_CONNECTED`; a service refusal → `GENERATION_FAILED` (not retried). B-roll: `media.generate-image` → `media.image-to-video` (Ken Burns) or a `motion.render-html` wrap. Use `16:9` / `9:16` for full-frame stills. |
| `project.inspect-footage` | `id`/`path`, `clipId` (optional) | Import checks per clip: `flat` (log / washed-out: suggests `project.set-clip-color --preset=flat-footage`), `backdrop` green\|blue\|none (suggests `project.set-clip-chroma-key`), `rotation` (vertical camera clips, shown upright), plus contrast / blackLevel / saturation and `suggestions:[{why, verb, args}]`. |
| `project.speech-map` | `id`/`path`, `minGapMs` (2500), `everyMs` (5000), `words` (bool) | The words the speaker stresses, from the audio (any language): `{ durationMs, moments:[{ atMs, text, phrase, strength 1-3, emphasis, reasons, suggest }] }`. Cues for zooms, keyword graphics and hits. Needs a transcript. [speech-timing.md](speech-timing.md) |
| `project.style-edit` | `id`/`path`, `style` (bold-short \| editorial-long \| clean-tutorial), `format` (short \| long; default from the aspect), `applyFixes`, `backdrop` (background behind a keyed-out speaker), `texts` ({ wordId: "English keyword" }), `endCard` ([line1, line2]), `accent` / `highlight` (colours; default the brand kit), `zooms` (true), `score` (true), `mood`, `dryRun`, `expectedRevision` | One call: inspect-footage (+ fixes), speech-map, native motion elements + silent zooms on the stressed moments in a house style, compose-soundtrack. Re-runs replace only what it placed. `dryRun` returns the plan. [motion-elements.md](motion-elements.md) |
| `project.add-motion-element` | `id`/`path`, `type` (keyword \| chip \| stamp \| count \| steps \| slam \| behind \| lowerThird \| highlight \| endCard \| progress \| iconPop), `content`, `atMs` \| `wordId`, `offsetMs?`, `durationMs?` \| `endWordId?`, `cueWordIds?` (steps), `style?` ({family, accent, highlight, text, font, size, outline}), `zone?` (auto \| top \| upper \| center \| lower \| bottom \| {x,y,w?}), `layer?` (front \| behind), `sound?` (auto \| none \| pop \| whoosh \| hit \| tick \| bell), `zIndex?`, `anchor?` (auto \| free), `expectedRevision?` | Native motion element drawn by the engine (preview = export). Returns `elementId`, `element`. [motion-elements.md](motion-elements.md) |
| `project.add-motion-elements` | `id`/`path`, `elements` ([{…add-motion-element fields}], max 200), `expectedRevision?` | Batch add, one revision, all or nothing (`element <i>: <reason>`). Returns `elementIds`. |
| `project.update-motion-element` | `id`/`path`, `elementId`, any add-motion-element field | Patch: style merges (null field = default), zone/layer/sound/zIndex null = default, atMs/wordId re-anchor keeping the length. Clears the style-edit origin. |
| `project.remove-motion-element` | `id`/`path`, `elementId`, `expectedRevision?` | Remove one (also `project.remove-region --regionType=motion-element`). |
| `project.list-motion-elements` | `id`/`path`, `type?`, `catalog?` | Placed elements with one-line summaries; `catalog` adds every type's content shape and defaults. |
| `project.compose-soundtrack` | `id`/`path`, `style` (talking-head \| short \| promo \| calm), `mood` (minor \| major), `bpm`, `sfx`, `useSpeech`, `cues` ([{atMs,strength,label}]), `musicVolume`, `takeOverSounds`, `dryRun`, `seed` | Score this edit from its own timeline (zooms, graphics, pops, cuts, speech, stressed words), render locally and place under the voice with ducking; clears zoom/overlay default sounds; replaces the previous score. Call last. `dryRun` returns the score. |
| `media.compose-soundtrack` | `score` (object) or `scorePath` (JSON file), `name` (optional) | Compose + render an original soundtrack (music, beats, SFX) from a score on the edit's own timeline: `cues`, `events` on cues, beat `sections` (chords, drums, bass, arp, melody), `master`. Synthesized locally (no key), deterministic, ~1 s per 15 s. Returns `{ audioPath, scorePath, durationMs, lufs, truePeakDb, events, cues, warnings }`; place with `project.add-audio`. Full format: [soundtrack.md](soundtrack.md). |
...
| `caption.set-template` | `id` \| `path`, `templateId` | Default `glowStack`. Static: `classic, modern, minimal, bold, spotlight, boxed, neon, colored, coloredWords, editorial, glowStack`. Animated (word by word as spoken): `kineticSlam, clipWipe, gradientPop, matrixDecode, glitchRgb, blendDifference`. Emphasis (key word, not spoken word; detects emphasis on apply): `hormoziEmphasis, tiltedBox, serifItalic, condensedCaps, scriptKeyword, wordBoxes, goldSerif, keywordBox, limeItalic`. More looks: `mutedBox, blackBar, oneWordPop, stickerTilt, spokenTilt, typewriterMono, softPill` (captions-metadata.md) (kebab-case aliases accepted). |
| `caption.set-style` | `id` \| `path`, color/font/stroke/positionY/wordsPerLine overrides, `highlightMode` (spoken\|emphasis\|both), `emphasisColor`, `emphasisBackgroundColor`, `emphasisScale` (0.5-2), `emphasisItalic`, `emphasisFontFamily`, `boxRotation` (0-15°) | Per-template style overrides. `positionY` is PERCENT of frame height from the top (0-100, default 85) — not a 0-1 fraction (fractions are auto-converted, but write percent). |
| `caption.mark-emphasis` | `id` \| `path`; `wordIds` + `emphasis` (default true) to mark/unmark by hand, or none to detect; `detect`, `reset`, `minGapMs` (1200), `everyMs` (2500) | The key words emphasis styles mark (`word.emphasis`). Detection = speech-map strength ≥ 2 moments + numbers; hand marks survive re-detection unless `reset`. |
...
- `media.generate-image`: `referenceImagePath` also accepts an https URL (uploaded to Higgsfield for you, or inlined for Replicate). `transparent=true` always returns a `.png`. `outputName` is slugified with a timestamp appended (same for `media.image-to-video`).
